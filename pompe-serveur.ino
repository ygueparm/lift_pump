...
for (int i = 0; i < MAX_ENTRIES; i++) {
    if (tempsReelDeFonctionnement[indiceAffichage] > 0 || tempsDepuisDernierDemarrage[indiceAffichage] > 0) {
        htmlContent += "<tr>";
        htmlContent += "<td>" + String(numeroIncrement) + "</td>"; // Ajoutez l'indice au début de la ligne
...
//delai depuis dernier demarrage moteur sur moniteur serie
        ...
        unsigned long jours = tempsEcoule_1 / JOUR;
        unsigned long heures = (tempsEcoule_1 % JOUR) / HEURE;
        unsigned long minutes = ((tempsEcoule_1 % JOUR) % HEURE) / MINUTE;
...
// autorisation de demarrer après la mise en securité du moteur si le temps d'attente est ecoulé
 ...
 if (tempsDebut != 0) { 
  if (etatContacteur == HIGH && !contacteurDeclenche) {
    ...
    tempsFinReel = millis();
    unsigned long tempsReel = tempsFinReel - tempsDebutReel;
